...
const newPlaylistBody = {
  name: "poopypants",
  public: true,
  description: "Derquan, SE12 Period 5, A whole bunch of songs about pure donkey sh*t"
...
async function createPlaylist(playlistData) {
  ...
  let response= await fetch (createPlaylistURL, options)
...
const songs = [
  ...
  "spotify:track:3LBM25lgxl9y549SiYxtSi",
  "spotify:track:6qvtoU0gbkaOcTx6mnVDgV",
  "spotify:track:4rHlfYRQw6ittRQXAlurk0"
];
...
async function addPlaylistSongs(songData) {
  ...
  let response = await fetch(addingSongsURL,options)
  let data= await response.json()
  console.log(data)
}
